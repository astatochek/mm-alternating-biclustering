...
def k_means(data_matrix: NDArray, n_clusters: int, *, eps=1.e-6):
    ...
    def get_loss(current_matrix: NDArray, updated_labels: NDArray, updated_centroids: NDArray) -> float:
        return np.mean([np.linalg.norm(current_matrix[i] - updated_centroids[updated_labels[i]]) for i in range(current_matrix.shape[0])])
...
def alternating_k_means_biclustering(
    ...
    def update_step(current_matrix: NDArray, preserved_labels: NDArray, processed_labels: NDArray) -> List[NDArray]:
        ...
        for j in range(n_clusters):
            ...
            I_j = np.where(I == j)
            centroid_j = np.array(np.zeros(I[I_j].size)) if rows_j.size == 0 else np.mean([row[I_j] for row in rows_j], axis=0)
    ...
    def assignment_step(
        ...
        for i in range(X.shape[0]):
            updated_labels[i] = np.argmin([np.linalg.norm(X[i][np.where(I == j)] - C[j])**2 / I[np.where(I == j)].size for j in range(n_clusters)])
        return updated_labels
    ...
    def alternate_iteration(
        ...
        I, J = preserved_labels, processed_labels
        # X = np.array([current_matrix.T[np.where(I == i)] for i in I]).T
        X = current_matrix

        def get_loss(new_centroids: List[NDArray]) -> float:
            C = new_centroids
            return np.sum([
                np.sum([np.linalg.norm(row[np.where(I == j)] - C[j])**2 / I[np.where(I == j)].size for row in X[np.where(J == j)]]) for j in range(n_clusters)
            ]) / X.shape[0]
...
row_labels, col_labels = alternating_k_means_biclustering(data, n_clusters, Dataset.BLOCK_DIAGONAL)
